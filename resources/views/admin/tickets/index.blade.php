@extends('layout')
@section('content')
<h1 class="m-5 text-center">Ticket index</h1>
<div class="container">
    ...
    <nav class="mb-1">
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link active" href="{{route('admin.tickets.index')}}">index</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="{{route('admin.tickets.handled')}}">Handled</a>
            </li>
        </ul>
    </nav>
    <div class="card mt-3">
                    ...
                    <div class="card">
                        ...
                        <div id="ticketcardbody{{$ticket->id}}" class="collapse" aria-labelledby="ticketcardhead{{$ticket->id}}">
                            <div class="card-body bg-light">
                                ...
                                <img id="ticketImg" class="ticket_img img-thumbnail" src="{{asset('storage/'.$ticket->file_path)}}" onclick="openImage()" alt="">
                            </div>
                            <div class="card-footer">
                                <p><span class="font-weight-bold">Opened by: </span> @empty ($ticket->created_by) Guest @else @empty ($ticket->createdBy) Deleted @else {{$ticket->createdBy->name}} @endempty @endempty <span class="font-weight-bold">At: </span>{{$ticket->updated_at}}</p>
                            </div>
                        </div>
                    </div>
                </div>
